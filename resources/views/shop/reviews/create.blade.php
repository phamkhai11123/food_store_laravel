    ...
    <div class="container mx-auto px-4 py-8">
        ...
        <div class="bg-white rounded-lg shadow-md overflow-hidden">
            <div class="px-6 py-4 bg-gray-100 border-b border-gray-200">
                <h1 class="text-2xl font-bold text-gray-800">Đánh giá sản phẩm</h1>
            </div>
            <div class="p-6">
                ...
                <form action="{{ route('shop.reviews.store', $orderItem->id) }}" method="POST" class="space-y-6">
                    @csrf
                    <div>
                            ...
                            <div class="flex items-center space-x-1">
                                @for ($i = 1; $i <= 5; $i++)
                                    <label class="cursor-pointer">
                                        <input type="radio" name="rating" value="{{ $i }}" class="hidden" {{ old('rating', 5) == $i ? 'checked' : '' }} required>
                                        <span class="star-icon text-3xl {{ old('rating', 5) >= $i ? 'text-yellow-400' : 'text-gray-300' }}">★</span>
                                    </label>
                                @endfor
                            </div>
                        </div>
                    ...
                    <div>
                        ...
                        <textarea name="comment" id="comment" rows="4" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="Chia sẻ trải nghiệm của bạn về sản phẩm này...">{{ old('comment') }}</textarea>
                        @error('comment')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
